...
void Grid::getVoxelID(Vec3f bmin, Vec3f bmax, Vec3f &l, Vec3f &r) {
  ...
  //Edge/Boundary cases
  rx = rx - (rx > lx && bmax.x() == rx * vx);
  ry = ry - (ry > ly && bmax.y() == ry * vy);
  rz = rz - (rz > lz && bmax.z() == rz * vz);
...
void Grid::initializeRayMarch(MarchingInfo &mi, const Ray &r, float tmin) {
  ...
  if (insideGrid(p))  //Case 1: ray starts inside grid
    ...
    if (mi.tmin < tmin) return; // ray does not intersect with grid
  ...
  mi.dtx = dir.x() == 0 ? 1e8 : vx / fabs(dir.x());
  mi.dty = dir.y() == 0 ? 1e8 : vy / fabs(dir.y());
  mi.dtz = dir.z() == 0 ? 1e8 : vz / fabs(dir.z());
